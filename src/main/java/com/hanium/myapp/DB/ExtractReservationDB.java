...
public class ExtractReservationDB {
	...
	public void ExtractLeftTime(String dep_ter, String arr_ter) throws Exception {
		...
		urlBuilder.append("?" + URLEncoder.encode("ServiceKey", "UTF-8")
				+ "=F4qo%2FN7dKHehezrw3sxLvLC5aFvf9L0DbYeDq9WTn0cQImSsFyckHpid%2BwJx1VrlT10NCUjakWo5jBRV7kURdw%3D%3D"); /*
																														 * Service
																														 * Key
																														 */
		urlBuilder.append(
				"&" + URLEncoder.encode("depTmnCd", "UTF-8") + "=" + URLEncoder.encode("010", "UTF-8")); /* 출발터미널코드 */
		urlBuilder.append(
				"&" + URLEncoder.encode("arrTmnCd", "UTF-8") + "=" + URLEncoder.encode("700", "UTF-8")); /* 도착터미널코드 */
		urlBuilder.append(
				"&" + URLEncoder.encode("numOfRows", "UTF-8") + "=" + URLEncoder.encode("999", "UTF-8")); /* 검색건수 */
	...
	private String extractResult(JSONArray items, String dep_ter, String arr_ter) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (deptmnnm.equals(dep_ter) && arrtmnnm.equals(arr_ter)) {
				text = "현재 " + dep_ter + "에서 " + arr_ter + "가고 있는 중이며 " + "현재 위치는 " + item.get("curlocnm") + "이며 앞으로 "
